...
class EventoProvider extends ChangeNotifier{
  ...
  Future<Response> post(Evento evento) async{
    const String endPoint ='/api/EventoLimpieza';
    final url = Uri.http(_host, endPoint);
    final client = RetryClient(http.Client());

    try {
      final response = await client.post(url, headers: {HttpHeaders.contentTypeHeader: 'application/json'}, body: eventoToJson(evento));
      print(response.body);
